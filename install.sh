...
for i in ~/.bashrc ~/.thymerc ~/.tmux.conf ~/.vimrc ~/.vim ~/.config/awesome/* ~/.vnc/* ~/.xinitrc; do
  echo "  removing $i"
  rm -f $i
done
...
for i in ~/.config ~/.config/awesome ~/.vnc; do
  if [ -d $i ]; then
...
for i in .bashrc .thymerc .tmux.conf .vimrc .xinitrc .vim; do
  echo "  linking $i"
  ln -s `pwd`"/$i" ~
done
